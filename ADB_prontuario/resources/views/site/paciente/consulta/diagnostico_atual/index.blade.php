@component('components.pages.menu_consulta')
    ...
    @slot('content_menu')
        @if ($page == 'cadastrar')
            <form action="{{route('diagnostico_atual.salvar', ['num_registro' => $dados->num_registro, 'num_USP' => $dados->num_USP])}}" method="post" class="row g-3 needs-validation" novalidate>
        @else
