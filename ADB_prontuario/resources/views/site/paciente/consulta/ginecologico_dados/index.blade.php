@component('components.pages.menu_consulta')
    ...
    @slot('content_menu')
        @if ($page == 'cadastrar')
            <form action="{{route('ginecologico_dados.salvar', ['num_registro' => $dados->num_registro, 'num_USP' => Auth::user()->num_USP])}}" method="post" class="row g-3 needs-validation form-consulta" novalidate>
        @else
            <form action="{{route('ginecologico_dados.atualizar', ['num_registro' => $dados->num_registro, 'num_USP' => Auth::user()->num_USP])}}" method="post" class="row g-3 needs-validation form-consulta" novalidate>
        @endif
            {{ csrf_field() }}
            @if ($page != 'cadastrar')
